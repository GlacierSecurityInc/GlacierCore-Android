...
langtoinclude="com es fr hu it no nl pl pt ro ru tr"
...
do
    ...
    mkdir -p src/glacierCore/res/values-$rlang/
    echo "$alang -> $rlang"
    tar -xv -C src/glacierCore/res/values-$rlang/ --strip-components 2 -f ics-openvpn.zip res/values-$alang/
done
