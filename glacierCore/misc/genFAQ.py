...
def main():
    ...
    loadstrres("src/glacierCore/res/values/strings.xml","default")
    
    #faqdom = dom.parse("src/glacierCore/res/layout/faq.xml")
    faqdom = open("src/glacierCore/java/com/blinkt/openvpn/fragments/FaqFragment.java").readlines()
    faq= genPage(faqdom,"default")

    open(faqpath + "/FAQ.html","w").write(template % {'content': faq})

    for directory in os.listdir("src/glacierCore/res"):
        if directory.startswith("values-") and directory.find("-sw")==-1 and not directory.startswith("values-v"):
            lang = directory.split("-",1)[1]
            print lang
            loadstrres("src/glacierCore/res/values-%s/strings.xml" % lang,lang)
